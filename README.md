...
제4회 국제 대학생 EV 자율주행 경진대회 (Advanced 자율주행 모빌리티 레이스 1/2 부문 최우수상).
...
|---|---|---|---|
| Camera publish | 웹캠 | `/camera/image_raw/compressed` | `camera_start` |
| Camera YOLO | `/camera/image_raw/compressed` | `/yolov8_pub` | `yolov8` |
| Livox clustering | `/livox/lidar` | `/jagyeong` | `Object_detect` |
| Livox-camera fusion | `/livox/lidar`, `/camera/image_raw/compressed`, `/yolov8_pub` | `/minjae` | `Object_detect` |
| Velodyne BEV detection | `/velodyne_points` | `/detected_objects` | `Object_detect` |

Livox-camera fusion: 토픽 동기화(LiDAR + 카메라 + YOLO) → LiDAR 포인트를 카메라 좌표로 투영 → YOLO bbox 매칭 포인트 필터링 → ROI + RANSAC 지면 제거 → 군집화 및 centroid → Kalman 추적.

## Layout

| 경로 | 내용 |
|---|---|
| `perception_ws/` | 인지 패키지, 커스텀 ultralytics(`yolov8_prune/`), conda `environment.yml` |
| `system_ws/` | 카메라 드라이버 `camera_start` |
...
| Package | Role |
|---|---|
| `Object_detect` | Livox-camera fusion (C++), Livox euclidean clustering, Velodyne BEV detection + OC-SORT |
...
| `livox_static_object` | Livox 정적 장애물 검출 (실험용) |

`system_ws/src`:

| Package | Role |
|---|---|
| `camera_start` | 웹캠 캡처 + undistort 퍼블리시 |
...
Conda env + 커스텀 ultralytics (기존 ultralytics가 있으면 먼저 `pip uninstall ultralytics`):
...
터미널마다 ROS와 각 workspace의 `devel/setup.bash`를 source 하고, YOLO 터미널은 `conda activate clothoid`.

```bash
roscore
roslaunch camera_start start.launch            # camera
roslaunch yolov8 yolo_detect.launch            # camera YOLO (fusion 보다 먼저)
roslaunch Object_detect detect.launch          # fusion + Velodyne BEV
rosrun Object_detect euclidean_clustering.py   # Livox clustering
```

`detect.launch`의 `model_path`와 `yolo_detect.py`의 `yolov8_prune` 경로는 절대 경로이므로 환경에 맞게 수정.
